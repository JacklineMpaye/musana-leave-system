                ...
                {pendingList.map((r: any, i: number) => {
                  ...
                  return (
                    <TableRow key={i} id={isHighlighted ? "highlighted-request" : undefined} className={isHighlighted ? "bg-primary/10 ring-2 ring-primary/30" : ""}>
                      ...
                      <TableCell className="text-sm">{r["Start Date"] || r.startDate || "-"} — {r["End Date"] || r.endDate || "-"}</TableCell>
                      <TableCell>{r.Days || r.days || "-"}</TableCell>
                      <TableCell><StatusBadge status={r.Status || r.status || "Pending"} /></TableCell>
                      <TableCell className="text-right space-x-2">
                        <Button size="sm" variant="outline" className="text-success border-success/30 hover:bg-success/10" onClick={() => setConfirmAction({ reqId, action: "approve" })}>
                          <CheckCircle className="w-4 h-4 mr-1" /> Approve
                        </Button>
                        <Button size="sm" variant="outline" className="text-destructive border-destructive/30 hover:bg-destructive/10" onClick={() => setConfirmAction({ reqId, action: "reject" })}>
            ...
            <Table>
              ...
              <TableBody>
                {allBalList.map((b: any, i: number) => (
                  <TableRow key={i}>
                    <TableCell className="font-medium">{b.Employee || b.employee || b.email || "-"}</TableCell>
                    <TableCell>{b["Leave Type"] || b.leaveType || b.type || "-"}</TableCell>
                    <TableCell>{b.Entitlement ?? b.entitlement ?? "-"}</TableCell>
      ...
      <Dialog open={!!confirmAction} onOpenChange={() => setConfirmAction(null)}>
        <DialogContent>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button variant={confirmAction?.action === "approve" ? "default" : "destructive"} disabled={actionMutation.isPending} onClick={() => { if (confirmAction) actionMutation.mutate({ requestId: confirmAction.reqId, action: confirmAction.action }); }}>
